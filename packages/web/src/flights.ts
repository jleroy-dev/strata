...
/** Where a flying thing is at `now`: position, vertical stretch, and whether it has landed. */
export function flightPose(
  f: FlightPath,
  now: number,
): { x: number; y: number; z: number; sy: number; k: number } {
...
export function arcPoints(f: FlightPath, lift: number, n = 40): THREE.Vector3[] {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    pts.push(
      new THREE.Vector3(
        ...
        lift + f.apex * Math.sin(Math.PI * k),
...
export class Flights implements SegmentSource {
  ...
  /** Largest distance between a landed ribbon's end and its mover, this frame. */
  gap = 0;
  ...
  *segments(now: number): Iterable<Segment> {
    for (const r of this.ribbons) {
      ...
      const flying = now < r.path.start + r.path.duration;
      const end = flying ? r.path.to : r.mover().clone().setY(0);
      const pts = arcPoints({ ...r.path, to: end }, r.lift);
  ...
  ribbon(
    ...
    this.ribbons.push({ path, color: color ?? NEUTRAL, lift, mover, ...(scar && { scar }) });
  }
  ...
  landed(
    ...
    this.effects.add({
      kind: 'ripple',
      at: new THREE.Vector3(path.to.x, 0.29, path.to.z),
      color,
      born: now,
      life: 600,
      size,
  ...
  update(now: number, hover: string | undefined, hoverTop: THREE.Vector3 | undefined): void {
    const gap = 0;
    ...
    if (landing && hoverTop) {
      const pts = arcPoints({ ...landing.path, to: hoverTop }, 0.3, 30);
