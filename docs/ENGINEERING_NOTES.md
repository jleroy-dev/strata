...
packages/
  ...
  web/      Vite + TS + three. Reads the event stream and nothing else; never touches git or
            ...
            `camera.ts` turns them into a matrix. Overview, Follow and Free write the same four
            numbers, so there is no handover between those three. Drone is the exception and owns
            an eye pose instead, a point in the air with a bearing and a tilt, converted to and
            from a `View` when it is entered and left. A focus is a point on the ground, and a
            gimbal that can raise its nose above the horizon has no ground point to name, so the
            fourth mode cannot be expressed in the other three's record without giving up the one
            shot it exists for. `drone.ts` integrates the flight and imports no `three`. `attention.ts`, `light.ts` and
...
| The map is never a place, at any distance                                                    | It is never a place at map altitude; Drone may look like one         | Seeing it drawn. At six cells over a district the towers are a skyline and it reads plainly as a city, which no amount of arguing on paper had produced. The abstraction turned out to be a property of the distance, so the rule is kept where the companion lives and spent in a mode entered on purpose.                                                                                                                                                                              |
| Colour says family from anywhere                                                             | Colour says family at map altitude; value carries at street level    | The same flight. Close up the key light desaturates the lit faces and the hue band stops separating, so the channel has a range and the design now names it rather than claiming the whole world.                                                                                                                                                                                                                                                                                        |
| A camera drone's gimbal points instantly                                                     | The gimbal is locked to the nose and turns at the machine's rate     | A preference, once both were flown. The free gimbal is the truer drone and was built first, but it lets the view and the travel disagree, and for a mode whose only job is to be enjoyed one honest machine beat an instant pointer bolted to a slow one. Aiming got worse and it was worth it.                                                                                                                                                                                          |
| Velocity is eased toward a heading                                                           | Thrust and drag integrate per axis in the drone's own frame          | The feel. Easing a normalised vector rotates it at fixed length, which has a slew rate and no mass at all: the drone could not drift, could not coast, and a turn under power dragged the picture behind the hand for a whole ramp.                                                                                                                                                                                                                                                      |
| The borrowing from games is three things and stops there                                     | It is four: a trigger was added, in Drone alone                      | Wanting one. The drone could move and do nothing else, which is a camera and not a toy, and the ask for something that fires was the right instinct about a real hole. The fence is that the weapons change nothing at all: no state, no stream, no score, and the reaction is the entire reward.                                                                                                                                                                                        |
| The panel is silent, and no sound is planned                                                 | Silent at rest; a trigger the reader pulled answers with a sound     | A distinction the original rule missed. The objection to sound was always that it arrives uninvited, and a weapon fired on purpose in a mode entered on purpose cannot. Impact research puts sound coherence among the three things that decide whether a hit lands, so a silent gun was fighting a third blind.                                                                                                                                                                         |
| Rotation is a critically damped spring on the angle                                          | It is a shaped rate curve on the lead, with no velocity kept         | Three bugs with one root. A spiral, a gimbal that went wild on a long sweep, and a blow-up whenever a frame ran long were all the same mistake: velocity state in the rotation. Rate as a pure function of the current error cannot overshoot, ring or run away, and it is less code than the spring it replaced.                                                                                                                                                                        |
| Drone writes the same four numbers as the other three modes                                  | Drone owns an eye pose and converts at the boundary                  | Its own altitude. A `focus` is a point on the ground, so the pitch had a floor to keep it finite, and at street level that floor forbids the one shot the mode is for: looking up at a tower whose cap is above the eye. The shared record was the weaker half of the claim and the mode is different enough to own its own.                                                                                                                                                             |
